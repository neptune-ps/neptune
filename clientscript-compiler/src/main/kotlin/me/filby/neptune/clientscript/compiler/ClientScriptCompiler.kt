...
class ClientScriptCompiler(
    ...
    fun setup() {
        ...
        addSymLoader(
            "component",
            type = ScriptVarType.COMPONENT,
            idSupplier = { str ->
                val parts = str.split(":", limit = 2)
                if (parts.size == 1) return@addSymLoader parts[0].toInt()
                val ifid = parts[0].toInt()
                val comid = parts[1].toInt()
                ifid shl 16 or comid
            },
        )
        ...
        addSymLoader(
            ...
            idSupplier = { str ->
                ...
                table shl 12 or (column shl 4) or (tuple + 1 and 0xf)
            },
        )
    ...
    private fun addSymLoader(name: String, type: Type, idSupplier: (str: String) -> Int = { str -> str.toInt() }) {
        addSymLoader(name = name, typeSupplier = { type })
    }
